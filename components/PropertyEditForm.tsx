...
const PropertyEditForm: React.FC<{ property: Property }> = ({ property }) => {
  const [formData, setFormData] = useState<PropertyAddForm>({
    ...
    amenities: Object.keys(PropertyAddFormBlankState.amenities).reduce(
      (
        acc: Record<AmenityKey, { value: boolean; text: string }>,
        value: string
      ) => {
        acc[value as AmenityKey] = {
          value: property.amenities.includes(value) ? true : false,
          text: value.toString(),
        };

        return acc;
      },
      {} as Record<AmenityKey, { value: boolean; text: string }>
    ),
    rates: {
  ...
  const handleFormSubmission = async (
    ...
    try {
      ...
      const res = await fetch(
        `/api/properties/${property._id.toString()}/edit`,
        {
          method: "PUT",
          body: formData,
        }
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        console.log("The updated property is: ", data);
        toast.success("Property updated successfully.");
        router.push(`/properties/${property._id.toString()}`);
      }
    } catch (error) {
      console.log(error);
